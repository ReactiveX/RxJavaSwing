		...
		SwingTestHelper.create().runInEventDispatchThread(new Action() {
			...
			public void run() {
				try {
					...
					assertEquals("Class 'java.lang.String' has not the expected signature to support change listeners in io.reactivex.swing.sources.ChangeEventSource",
							ex.getMessage());
				}
				...
				try {
					...
					assertEquals("Class '" + changeEventSource.getClass().getName() + "' has not the expected signature to support change listeners in io.reactivex.swing.sources.ChangeEventSource",
							ex.getMessage());
				}
			}
		}).awaitTerminal();
